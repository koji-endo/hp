...
window.onload = function() {
  ...
  vUniLocation[4] = gl.getUniformLocation(vPrg, "target_value");
  vUniLocation[5] = gl.getUniformLocation(vPrg, "target_index");
  ...
  // フレームバッファの生成
  var backBuffer = create_framebuffer(
    TEXTURE_WIDTH,
    TEXTURE_HEIGHT,
    gl.UNSIGNED_BYTE
  );
  var frontBuffer = create_framebuffer(
    TEXTURE_WIDTH,
    TEXTURE_HEIGHT,
    gl.UNSIGNED_BYTE
  );
  ...
  function render() {
    ...
    let target_index = count % (TEXTURE_WIDTH * TEXTURE_HEIGHT);
    let target_value = 0.5 * (1.0 + Math.sin((count * Math.PI) / 180.0));
    ...
    gl.uniform1f(vUniLocation[4], target_value);
    gl.uniform1f(vUniLocation[5], target_index);
    gl.drawArrays(gl.TRIANGLE_STRIP, 0, position.length / 3);
